    ...
    const redirect = async () => {
        ...
        if (typeof window !== "undefined"){
            try {
                ...
                setDisabled(false)
                window.location.replace(`${merchantUrl}?cmd=_paynow&receiver=${merchantId}&item_name=Buy+Tickets&amount=${price}.00&return_url=${process.env.PAYMENT_URL}%2Fpayment-success%3FpartyId=${partyId}%26eventId=${state.partyForm.eventId}%26tickets=${numberOfTicket}&cancel_url=${process.env.PAYMENT_URL}%2Fpayment-failed`);
            }catch(err){
                console.log(err)
            }
        }
